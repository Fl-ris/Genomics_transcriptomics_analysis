...
Voor dit onderzoek wordt de expressie van een aantal genen die later bescreven worden vergeleken tussen twee groepen muizen: oude en jonge muizen.
...
**Verschillen tussen het originele onderzoek en dat van ons:** De onderzoekers van het artikel hebben een verouderd muis referentiegenoom gebruikt, (mm10) deze vervangen we door mm39.
Dit is de niewste versie en wordt later in dit verslag uitgebereid beschreven.

### Workflow (Transcriptomics):

Korte samenvatting van het orginele onderzoek: - Invloed van leeftijd op ruimtelijke chromatine veranderingen en de invloed hiervan op de expressie van PAX5, Ebf1 en FOXO1.
- Verschil in gen expressie ebf1 tussen oude en jonge muizen.
(verminderde interactie tussen ebf1 promoter en zijn enhancers) - Verminderde ebf1 expressie leidt tot functieverlies B cellen.
- Verplaatsing van het ebf1 gen van compartiment A naar B.

Experimentele opzet: RNA-seq:\
- Voorloper B-cellen\
...
1.  Kwaliteitscontrole (FastQC)
2.  Trimmen met Trimmomatic
3.  Kwaliteitscontrole na trimmen (FastQC)
...
8.  Visualiseren (in de volgende dia besproken...)

Dimensiereductie om verschil tussen groepen zichtbaar te maken: - PCA tussen de twee groepen

Dit onderzoek tracht de volgende onderzoeksvraag te beantwoorden: Is er een expressieverschil van Ebf1, Pax5 en Foxo1 tussen de jonge en oude muizen?

### Gebruikte libraries:

De volgende R libraries en commandline tools zijn gebruikt voor de analyse.
Alle geladen libraries voor dit project zijn verkregen met het R commando: "(.packages())"
...
![Na trimmen GC content](/home/floris/Documenten/git_repo/Genomics_transcriptomics_analysis/analysis/Floris/Data/Na_trimmen_gc.png "Na trimmen GC content") De kwaliteit van de data lijkt voldoende te zijn, voor meer informatie zie logboek Ivar, Storm en Jarno.
...
De namen van de samples zijn erg lang en bevatten het gehele pad naar de bestanden, daarom worden de kolommen van de samples met de onderstaande code hernoemd naar de SSR namen.
...
**Dataframe voor DESeq2** Om later DESeq2 uit te voeren moet de data in een bepaald formaat staan.
...
Een MA-plot visualiseert de relatie tussen de log ratio en de gemiddelde expressie waarden.
...
Pax5 is een transcriptiefactor die enkel in B-cellen tot expressie komt.
...
Wings apart-like protein homolog (WAPL) is een gen dat resulteert in een eiwit dat betrokken is bij loslaten van Cohesine van chromatine.
...
Een volcano plot weergeeft alle genen maar enkel de genen die statistisch significant zijn en een hoge fold change hebben worden rood gekleurd.
De genen die wel statistisch significant zijn maar geen significante fold change hebben zijn blauw gekleurd.
Daarnaast zijn alle genen die geen statistisch significante p-waarde hebben en ook geen significante fold change hebben grijs gekleurd.
De -log10(p-waarde) is op de y-as weergeven en op de x-as staat de log2(fold change).
...
het volcano plot weergeeft onderandere de uitschieter Gpnmb, hiervan is ook het verschil tussen de groepen gevisualiseerd.
(Chunk 30)

### Pathview:

Wanneer er veel DEGs zijn is het moeilijk om dit te visualiseren en nuttige informatie uit de verkregen plots te halen.
Vaak hebben de genen invloed op elkaar, dit is zichtbaar te maken met een pathway analyse.
Hier wordt gebruik gemaakt van pathview.
Een pathway is een serie moleculaire interacties die van elkaar afhankelijk zijn.
Dit betekend ook dat als er aan het begin van een pathway iets verkeerd gaat alle stappen er achter niet meer uitgevoerd kunnen worden.
...
Omdat de sequence dat afkomstig is van B-cellen is het B cell receptor pathway extreem relevant, enkele surface receptors zoals de CDs worden gebruikt als marker om achter het ontwikkelstadium van de b-cel te komen.
Wanneer er grote expressie verschillen zijn tussen de jonge en oude groep er er meer voorloper-b-cellen aanwezig zijn bij een van de groepen kan dit verklaard worden door expressie verschillen van de genen die verantwoordelijk zijn voor het maken van de receptoreiwitten die als marker gebruikt worden.
...
Er is overlap tussen de surface eiwitten van B- en T-lymfocyten.
Dus ondanks dat de gesequencede cellen B-cellen zijn en geen T-cellen is het volgende pathway ook interesant.
Op de onderstaande pathway afbeelding is downregulatie van CD19 zichtbaar in oude muizen tegenover de jong muizen.
Dit is een transmembraan glycoproteine die betrokken is bij de ontwikkeling van B-cellen.
...
De twee groepen die vergeleken worden zijn jonge en oude muizen, daarom is het longevity pathway dat enkele genen weergeeft die betrokken zijn bij het verouderingsproces ook belangrijk.
Niet alle genen die betrokken zijn bij dit proces zijn hierop afgebeeld omdat het niet bekend is welke genen verantwoordelijk zijn.
Foxo is downregulated in de oude muizengroep tegenover de jonge groep.
Van dit gen is bekend dat het levensduur gerelateerd is.
...
Longevity regulating pathway - multiple species: Een vergelijkbaar pathway die een overzicht geeft tussen longegevity pathways tussen verschillende organismes.
...
Omdat B-cellen uit hematopoëtische cellen voortkwamen, en de voorloper B-cellen die hiervoor gebruikt zijn nog dichter in de buurt van deze hematopoëtische stamcellen zijn is het volgende pathway ook interessant: Weer is CD19 upregulated in de jongere muizen wat overeenkomt met de bestaande literatuur: Bron: <https://pmc.ncbi.nlm.nih.gov/articles/PMC3814412/>
