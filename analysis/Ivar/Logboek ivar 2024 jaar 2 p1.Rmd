...
Mappen is het procces

#### Alignen

### Freebayes variant calling
Voor een voledige uitleg van Freebayes varriant calling zie het logboek van Jarno en Floris 
...
## Donderdag
Begin gemaakt pricipale componenten
jarno geholpen met
...
```{r}
an_var <- read.delim("/Users/ivarl/Desktop/annotated_variants.vcf", skip = 125, stringsAsFactors = FALSE)
```


```{r}
#unique(an_var$X.CHROM)
vcf_list <- strsplit(an_var$INFO,split = ";")
# split info voor allel freq

# maak matrix
info_matrix <- do.call(rbind, vcf_list)
# verwijder AF= voor typecasting
info_matrix <- gsub("AF=", replacement = "", x = info_matrix)
allel_freq <- as.numeric(info_matrix[,2])

# irange object op 1 positie want het is snip
irange_object <- IRanges(start = an_var$POS,
                         end = an_var$POS,
                        names = an_var$X.CHROM
                         )
# verander naar grange
(grange_object <- GRanges(seqnames = an_var$X.CHROM,
                          ranges = irange_object))
```
